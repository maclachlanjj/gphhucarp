...
//            System.out.println("Key from: " + entry.getKey().getFrom() + " key to: " + entry.getKey().getTo());
    ...
    public DecisionProcessState deepClone(ReactiveDecisionSituation rds, int seedMod){
        ...
        List<Arc> unassignedTasks_clone = new LinkedList<>(unassignedTasks);
    ...
    public List<Boolean> returned;
    public List<Boolean> getReturned(){
        return returned;
    }
    public void setReturned(List<Boolean> r) { returned = r; }
    public void resetReturned(){
        returned = new ArrayList<>();
        for(int i = 0; i < solution.getRoutes().size(); i++) returned.add(false);
    }
}
